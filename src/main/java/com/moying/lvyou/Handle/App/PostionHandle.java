...
public class PostionHandle {
  ...
  public static void List(RoutingContext request, Pool client) {
    client.query("SELECT * FROM postion order by pid desc limit 100").execute(ar -> handleQueryResult(request,true, ar));
  }
  ...
  public static void Post(RoutingContext request, Pool client) {
    ...
    lat        varchar(20) default '' not null comment '维度',
    lng        varchar(20) default '' not null comment '经度',
    ` range`   int         default 0  not null comment '打卡距离',
    content    text                   null comment '景点介绍',
    detail     json                   null comment '景点细节',
    createtime datetime
     */
    client.preparedQuery("INSERT INTO postion set name=?, lat=?,lng=?,`range`= ?,content= ?,detail= ?,createtime= ?")
      .execute(Tuple.of(
        entity.getName(),
        entity.getLat(),
        entity.getLng(),
  ...
  public static void Put(RoutingContext request, Pool client) {
    ...
    var body = Json.encodePrettily(request.body().asJsonObject());
    var entity = Json.decodeValue(body, Postion.class);
    client.preparedQuery("update postion set name=?, lat=?,lng=?,`range`= ?,content= ?,detail= ? where pid=?")
      .execute(Tuple.of(
        entity.getName(),
        entity.getLat(),
        entity.getLng(),
