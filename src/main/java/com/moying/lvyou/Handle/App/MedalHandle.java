...
public class MedalHandle {
  ...
  public static void Page(RoutingContext request, Pool client) {
    ...
    // language=SQL
    var sql = "select * from medal";
    // language=SQL
    var countsql = "select count(1) as records from medal";
  ...
  public static void Post(RoutingContext request, Pool client) {
    ...
    var entity = Json.decodeValue(body, Medal.class);
    /*
    roadid     int          default 0  not null comment '线路id',
    userid     int          default 0  not null comment '用户id',
    pic        varchar(200) default '' not null comment '勋章图片',
  ...
  public static void Put(RoutingContext request, Pool client) {
    ...
    var body = Json.encodePrettily(request.body().asJsonObject());
    var entity = Json.decodeValue(body, Medal.class);
    client.preparedQuery("update medal set roadid=?, userid=?,pic=?,isshow= ? where mid=?")
      .execute(Tuple.of(
        entity.getRoadid(),
        entity.getUserid(),
        entity.getPic(),
