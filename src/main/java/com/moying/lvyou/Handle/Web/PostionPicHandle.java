...
public class PostionPicHandle {
  ...
  public static void List(RoutingContext request, Pool client) {
    var pid = Integer.parseInt(request.queryParams().get("pid"));
    client.preparedQuery("SELECT * FROM postion_pic where pid=? order by picid desc limit 100").execute(Tuple.of(pid),ar -> handleQueryResult(request,true, ar));
  }
  ...
  public static void Get(RoutingContext request, Pool client) {
    ...
    if (id == 0) {
      sendErrorResponse(request, JsonResult.fail("pid 参数不能为空"));
      return;
    }
  ...
  public static void Put(RoutingContext request, Pool client) {
    ...
    var body = Json.encodePrettily(request.body().asJsonObject());
    var entity = Json.decodeValue(body, PostionPic.class);
    client.preparedQuery("update postion_pic set title=?,pic=?,sn= ? where pid=?")
      .execute(Tuple.of(
        entity.getTitle(),
        entity.getPic(),
        entity.getSn(),
