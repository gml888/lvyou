...
import com.moying.lvyou.Entity.BbsCategory;
...
public class BbsCategoryHandle {

  public static void List(RoutingContext request, Pool client) {
    client.query("SELECT * FROM bbs_category order by cid desc limit 100").execute(ar -> handleQueryResult(request,true, ar));
  }
  ...
  public static void Page(RoutingContext request, Pool client) {
    ...
    // language=SQL
    var sql = "select * from bbs_category";
    // language=SQL
    var countsql = "select count(1) as records from bbs";
    StringBuilder where = new StringBuilder(" where isshow=1 ");
    var orderby = pageParms.getOrderby().isEmpty() ? " order by cid desc" : " order by " + pageParms.getOrderby();//排序
    //下面重组动态查询
  ...
  public static void Get(RoutingContext request, Pool client) {
    ...
    client.preparedQuery("SELECT * FROM bbs_category WHERE cid = ?")
  ...
  public static void Post(RoutingContext request, Pool client) {
    ...
    var entity = Json.decodeValue(body, com.moying.lvyou.Entity.BbsCategory.class);
    if (entity.getName() == null || entity.getPic().isEmpty()) {
      sendErrorResponse(request, JsonResult.fail("标题及图片不能为空"));
      return;
    }
    client.preparedQuery("INSERT INTO bbs_category set name=?,pic=?,isshow=?,createTime= ?")
      .execute(Tuple.of(
        entity.getName(),
        entity.getPic(),
        1,
  ...
  public static void Delete(RoutingContext request, Pool client) {
    ...
    client.preparedQuery("DELETE FROM bbs_category WHERE cid = $1")
  ...
  public static void Put(RoutingContext request, Pool client) {
    ...
    var body = Json.encodePrettily(request.body().asJsonObject());
    var entity = Json.decodeValue(body, BbsCategory.class);

    client.preparedQuery("update bbs_category set name=?, pic=? where cid=?")
      .execute(Tuple.of(
        entity.getName(),
        entity.getPic(),
