...
public class ManageHandle {
  ...
  public static void List(RoutingContext request, Pool client) {
    client.query("SELECT * FROM manage order by id desc limit 100").execute(ar -> handleQueryResult(request,true, ar));
  }
  ...
  public static void Page(RoutingContext request, Pool client) {
    ...
    // language=SQL
    var countsql = "select count(1) as records from manage";
    StringBuilder where = new StringBuilder();
    var orderby = pageParms.getOrderby().isEmpty() ? " order by id desc" : " order by " + pageParms.getOrderby();//排序
    //下面重组动态查询
        ...
        if (!queryParms.get(key).isEmpty()) {
          if(queryParms.get(key).matches("-?\\d+")){
            if(Integer.parseInt(queryParms.get(key))>0){
              where.append(" or id = ?");
              tuple.addInteger(Integer.parseInt(queryParms.get(key)));
            }
          }
  ...
  public static void Get(RoutingContext request, Pool client) {
    ...
    client.preparedQuery("SELECT * FROM manage WHERE id = ?")
  ...
  public static void Post(RoutingContext request, Pool client) {
    ...
    client.preparedQuery("SELECT * FROM manage WHERE username = ?")
      .execute(Tuple.of(entity.getUsername()), ar -> {
        if (ar.succeeded()) {
          if (ar.result().iterator().hasNext()) {
            sendErrorResponse(request, JsonResult.fail("用户名已存在"));
          } else {
            client.preparedQuery("INSERT INTO manage set username=?, password= ?,name= ?,email= ?,roleId= ?, status= ?,createTime= ?")
              .execute(Tuple.of(
                entity.getUsername(),
                Utils.md5(entity.getPassword()),
                entity.getName(),
                entity.getEmail(),
                entity.getRoleId(),
  ...
  public static void Delete(RoutingContext request, Pool client) {
    ...
    client.preparedQuery("DELETE FROM manage WHERE id = $1")
  ...
  public static void Put(RoutingContext request, Pool client) {
    ...
    var body = Json.encodePrettily(request.body().asJsonObject());
    var entity = Json.decodeValue(body, Manage.class);
    client.preparedQuery("update manage set name=?, email=?,roleId=?,status=? where id=?")
      .execute(Tuple.of(
        entity.getName(),
        entity.getEmail(),
        entity.getRoleId(),
  ...
  public static void Login(RoutingContext request, JWTAuth jwtAuth, Pool client) {
    ...
    client.preparedQuery("select * from manage where username=? and password=?")
      .execute(Tuple.of(
        entity.getUsername(),
        Utils.md5(entity.getPassword())), ar -> {
        if (ar.succeeded()) {
          if(ar.result().iterator().hasNext()){
            var userid = ar.result().iterator().next().getInteger("id");
            // 生成 JWT 令牌
            JsonObject payload = new JsonObject()
              .put("sub", userid + "_" + username)
