...
public class BbsHandle {
  ...
  public static void Page(RoutingContext request, Pool client) {
    ...
    // language=SQL
    var countsql = "select count(1) as records from bbs";
  ...
  public static void Get(RoutingContext request, Pool client) {
    ...
    client.preparedQuery("SELECT * FROM bbs WHERE bid = ?")
  ...
  public static void Post(RoutingContext request, Pool client) {
    ...
    try {
      var entity = JsonUtil.deserialize(body, Bbs.class);
        ...
        .execute(Tuple.of(entity.getUserid()), ar -> {
          if (ar.succeeded()) {
            if (ar.result().size() == 0) {
              ...
              client.preparedQuery("INSERT INTO bbs set userid=?,cid=?, title=?,pic=?,piclist=?,content=?,createTime= ?")
                .execute(Tuple.of(
                  entity.getUserid(),
                  entity.getCid(),
                  entity.getTitle(),
      ...
      .execute(Tuple.of(entity.getUserid()), ar -> {
          if (ar.succeeded()) {
            if (ar.result().size() == 0) {
              ...
              client.preparedQuery("update bbs set userid=?,cid=?, title=?,pic=?,piclist=?,content=? where bid=?")
                .execute(Tuple.of(
                  entity.getUserid(),
                  entity.getCid(),
                  entity.getTitle(),
                  ...
                  entity.getPiclist(),
                  entity.getContent(),
                  id), ar1 -> handleUpdateResults(request, ar1));
            }
          }
        });
    }catch (Exception e) {
