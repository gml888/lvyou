...
public class RoadPostionOptionHandle {
  ...
  public static void List(RoutingContext request, Pool client) {
    var rpid = Integer.parseInt(request.queryParams().get("rpid"));
    client.preparedQuery("SELECT * FROM road_postion_option where rpid=? order by sn desc limit 100").execute(Tuple.of(rpid), ar -> handleQueryResult(request, true, ar));
  }
  ...
  public static void Post(RoutingContext request, Pool client) {
    ...
    client.preparedQuery("INSERT INTO road_postion_option set rpid=?, title=?,pic=?, url=?, content=?, sn=?, isshow=?, createtime= ?")
      .execute(Tuple.of(
        entity.getRpid(),
        entity.getTitle(),
        entity.getPic(),
  ...
  public static void Delete(RoutingContext request, Pool client) {
    ...
    client.preparedQuery("DELETE FROM road_postion_option WHERE rpoid = ?")
  ...
  public static void Put(RoutingContext request, Pool client) {
    ...
    var body = Json.encodePrettily(request.body().asJsonObject());
    var entity = Json.decodeValue(body, RoadPostionOption.class);
    client.preparedQuery("update road_postion_option set title=?,pic=?, content=?, sn=?, isshow=? where rpoid=?")
      .execute(Tuple.of(
        entity.getTitle(),
        entity.getPic(),
        entity.getContent(),
